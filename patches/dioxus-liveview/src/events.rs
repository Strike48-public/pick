...
impl HtmlEventConverter for SerializedHtmlEventConverter {
    ...
    fn convert_mounted_data(&self, event: &PlatformEventData) -> MountedData {
        // Gracefully handle events from non-liveview contexts (e.g. native Dioxus WebView
        // on Android where the global event converter is shared between native and liveview).
        match event.downcast::<LiveviewElement>().cloned() {
            Some(el) => el.into(),
            None => {
                // Return a no-op MountedData — the element won't support queries
                // but at least it won't crash the app.
                LiveviewElement::new(
                    dioxus_core::ElementId(0),
                    crate::query::QueryEngine::default(),
                )
                .into()
            }
        }
